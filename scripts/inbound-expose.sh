...
#   AKS_LOCATION        optional   the cluster's region (default westus3);
...
while :; do
  ...
  if [ -n "$ip" ]; then
    ...
    # keeps waiting.
    code=$(curl -sS -o /dev/null -m 10 -w '%{http_code}' "https://$FQDN/" 2>/dev/null || true)
    if [ "$code" = 404 ]; then
      break
    fi
  fi
...
post=$(curl -sS -o /dev/null -m 10 -w '%{http_code}' -X POST --data '{}' "https://$FQDN/hook/slack-events" || true)
case "$post" in
