...
| Cut a real release: an agent drafts the notes from what merged, proposes the branch and the builds, and a human approves each call by version and repository | [release-agent.md](release-agent.md) | `make release-secret`, `make ado-secret`, `make govern-release`, `make release-bind`, `make release` |
