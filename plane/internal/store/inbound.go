...
func (s *Store) CredentialByName(ctx context.Context, name string) (Credential, error) {
	...
	err := s.pool.QueryRow(ctx,
		`SELECT name, cap_cents, cap_tokens FROM credential WHERE name = $1`, name).
		Scan(&c.Name, &c.CapCents, &c.CapTokens)
...
func (s *Store) AdmitInboundEvent(ctx context.Context, hook, credential, delivery, agent string) (eventID, grantID string, err error) {
	...
	err = tx.QueryRow(ctx,
		`INSERT INTO inbound_audit (hook, credential_name, delivery_id, decision, status, agent)
		 VALUES ($1, $2, $3, 'admitted', 202, $4) RETURNING id`,
		hook, credential, delivery, agent).Scan(&eventID)
...
func (s *Store) InboundAudit(ctx context.Context, hook string, limit int) ([]InboundAuditEntry, error) {
	...
	rows, err := s.pool.Query(ctx,
		`SELECT id, hook, credential_name, delivery_id, decision, status, detail, agent, input_tokens, output_tokens, created_at
		 FROM inbound_audit
		 WHERE ($1 = '' OR hook = $1)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&e.ID, &e.Hook, &e.CredentialName, &e.DeliveryID, &e.Decision, &e.Status,
			&e.Detail, &e.Agent, &e.InputTokens, &e.OutputTokens, &e.CreatedAt); err != nil {
			return nil, err
		}
		out = append(out, e)
	}
